...
def generate_referral_code(instance):
    Klass = instance.__class__
    referral_code = generate_unique_code(6)
    if Klass.objects.filter(referral_code = referral_code).exists():
        return generate_referral_code(instance)
